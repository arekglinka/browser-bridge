// MAIN world interceptor — standalone IIFE.
// CRITICAL: No chrome.* APIs, no ES modules, no WASM imports here.
// Runs in the page's MAIN JavaScript context where only DOM APIs exist.
(function () {
  "use strict";

  var _tokenBuffer = [];
  var _bridgeReady = false;
  var _dedupMap = Object.create(null);
  var _DEDUP_TTL = 5000;
  ...
  function extractTokenFromBody(body) {
    ...
    if (token) return { type: token.split("-")[0], token: token };
    return null;
  }
  ...
  function emitToken(detail) {
    ...
    if (_bridgeReady) {
      window.dispatchEvent(
        new CustomEvent("__KB_TOKEN__", { detail: detail })
      );
    } else {
  ...
  window.fetch = function (input, init) {
    ...
    if (typeof Request !== "undefined" && input instanceof Request) {
      try {
        ...
        // sealed/frozen Headers throw on .get()
      }
    }
